아, 제가 직전 답변에서 수정된 전체 코드를 안내해 드리는 과정에서, 앞단에 덧붙인 설명 글까지 실수로 파이썬 코드 창 안에 함께 밀어 넣어 버렸네요! 🤦‍♀️

파이썬 엔진이 코드 맨 첫 줄에 있는 `🏢` 이모지와 한글 설명 글을 읽다가 문법 에러(`SyntaxError`)를 낸 것입니다.

설명 글을 완벽하게 걷어내고, **곧바로 복사해서 붙여넣기만 하면 정상 작동하는 순수 파이썬 소스코드**로 다시 정제해 드립니다.
