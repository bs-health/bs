...
with st.sidebar:
    ...
    # 📅 [핵심 변수] 대시보드 상단에서 유저가 선택한 날짜 (텍스트 형태: YYYY-MM-DD)

    date_str_key = str(today_kst)



    # 💡 파일에서 읽어온 데이터 중 '현재 선택된 날짜'에 조치한 현장 이름만 추출해서 사용합니다.
        ...
        if not db_master.empty:
            ...
            if missing_sites_for_admin:
                ...
                if st.button("선택 현장 '실시'로 강제 전환 및 저장"):
                    ...
                    # 💡 저장할 때 뒤에 날짜 정보를 결합하여 중복 및 다음 날짜 연동 방지
...
if st.session_state.current_tab == "📊 총괄 브리핑":
    ...
    # 💡 오늘 날짜용 액티브 수동 완료 리스트만 총괄 브리핑에 대입합니다.
...
# ------------------------------------------
    ...
    if db_master.empty:
        ...
        # 💡 오늘 자 기준 수동 완료 데이터 세트만 병합하여 매칭률 계산 및 리스트 연동
                    ...
                    if not filtered_missing.empty:
                        ...
                        for idx, row_missing in filtered_missing.iterrows():
                            ...
                            if is_admin:
                                ...
                                # 관리자 세션: 클릭 시 현재 대시보드 날짜 태그를 명확히 붙여서 set 구조에 보관
                ...
                with st.expander(f"✅ 실시 완료 ({len(submitted)}곳)", expanded=False):
                    ...
                    if not submitted.empty:
                        ...
                        for idx, row_submitted in submitted.iterrows():
                            ...
                            # 💡 오늘 자 날짜 꼬리표가 정확히 들어맞는 수동 변경 데이터 패킷만 미실시 복구 버튼 활성화
